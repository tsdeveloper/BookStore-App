...
export class AssuntoService {
  ...
  getAssuntos(useCache = true): Observable<Pagination<Assunto[]>> {
    ...
    if (this.assuntoCache.size > 0 && useCache) {
      if (this.assuntoCache.has(Object.values(this.assuntoParams).join('-'))) {
        this.pagination = this.assuntoCache.get(Object.values(this.assuntoParams).join('-'));
    ...
    if (this.assuntoParams.codAs > 0) params = params.append('codAs', this.assuntoParams.codAs);
    ...
    if (this.assuntoParams.search) params = params.append('search', this.assuntoParams.search);

    return this.http.get<Pagination<Assunto[]>>(this.baseUrl + 'assunto/all', {params}).pipe(
      map(response => {
        this.assuntoCache.set(Object.values(this.assuntoParams).join('-'), response)
  ...
  update(value: Assunto) {
    return this.http.put(this.baseUrl + `assunto/editar/${value.codAs}`, value);
  }
  ...
  getAssunto(id: number) {
    const assunto = [...this.assuntoCache.values()]
