...
export class AutorService {
  ...
  getAutors(useCache = true): Observable<Pagination<Autor[]>> {
    ...
    if (this.autorCache.size > 0 && useCache) {
      if (this.autorCache.has(Object.values(this.autorParams).join('-'))) {
        this.pagination = this.autorCache.get(Object.values(this.autorParams).join('-'));
    ...
    if (this.autorParams.codAu > 0) params = params.append('codAu', this.autorParams.codAu);
    ...
    if (this.autorParams.search) params = params.append('search', this.autorParams.search);

    return this.http.get<Pagination<Autor[]>>(this.baseUrl + 'autor/all', {params}).pipe(
      map(response => {
        this.autorCache.set(Object.values(this.autorParams).join('-'), response)
  ...
  getAutor(id: number) {
    const autor = [...this.autorCache.values()]
