...
export class LivroService {
  ...
  getLivros(useCache = true): Observable<Pagination<Livro[]>> {
    ...
    if (this.livroCache.size > 0 && useCache) {
      if (this.livroCache.has(Object.values(this.livroParams).join('-'))) {
        this.pagination = this.livroCache.get(
          Object.values(this.livroParams).join('-')
        );
    ...
    if (this.livroParams.codL > 0)
      params = params.append('codL', this.livroParams.codL);
    if (this.livroParams.edicao > 0)
      params = params.append('edicao', this.livroParams.edicao);
    if (this.livroParams.editora)
      params = params.append('editora', this.livroParams.editora);
    if (this.livroParams.anoPublicacao)
      params = params.append('anoPublicacao', this.livroParams.anoPublicacao);
    ...
    if (this.livroParams.search)
      params = params.append('search', this.livroParams.search);

    return this.http
      .get<Pagination<Livro[]>>(`${this.baseUrl}/livro/all`, { params })
      .pipe(
        map((response) => {
          this.livroCache.set(
            Object.values(this.livroParams).join('-'),
  ...
  getLivro(id: number, useCache: boolean = false) {
    if (useCache) {
      const livro = [...this.livroCache.values()].reduce(
        (acc, paginatedResult) => {
          return { ...acc, ...paginatedResult.data.find((x) => x.codL === id) };
        },
        {} as Livro
      );
